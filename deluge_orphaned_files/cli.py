...
if "--list-env" in sys.argv:
    try:
        ...
        masked: dict[str, str] = {}
        for key, value in _cfg.model_dump(
            mode="python", exclude_defaults=False
        ).items():
            if value is None:
                display = "NOT SET"
...
class AppConfig(BaseSettings):
    ...
    deletion_consecutive_scans_threshold: int = Field(
        default=7, alias="DELETION_CONSECUTIVE_SCANS_THRESHOLD"
    )
    ...
    smtp_to_addrs: List[str] = Field(
        default_factory=list, alias="_INTERNAL_SMTP_TO_LIST"
    )
    smtp_use_ssl: bool = Field(default=True, alias="SMTP_USE_SSL")

    deluge_autoremove_label: str = Field(
        default="othercat", alias="DELUGE_AUTOREMOVE_LABEL"
    )
    ...
    def _populate_parsed_lists(self) -> "AppConfig":
        ...
        if isinstance(effective_ext_str, str):
            self.extensions_blacklist = [
                item.strip().lower()
                for item in effective_ext_str.split(",")
                if item.strip()
            ]
        ...
        if self.raw_smtp_to_str:
            self.smtp_to_addrs = [
                addr.strip() for addr in self.raw_smtp_to_str.split(",") if addr.strip()
            ]
        else:
            self.smtp_to_addrs = []
        return self
...
def init_sqlite_cache(db_path: Path) -> None:
    ...
    try:
        ...
        logger.info(
            "SQLite cache schema (tables and main view) initialized/verified successfully."
        )
    except sqlite3.Error as e:
        logger.error(
            f"SQLite error during schema initialization (tables/main view) in {db_path}: {e}"
        )
        raise
    except Exception as e:
        logger.error(
            f"Unexpected error during schema initialization (tables/main view) in {db_path}: {e}"
        )
        raise
    ...
    try:
        ...
        logger.error(
            f"SQLite error creating 'view_files_eligible_for_deletion' in {db_path}: {e}"
        )

    except Exception as e:
        logger.error(
            f"Unexpected error creating 'view_files_eligible_for_deletion' in {db_path}: {e}"
        )
    ...
    try:
        ...
        logger.error(
            f"SQLite error creating 'vw_autoremove_candidates_latest_scan' in {db_path}: {e}"
        )
    except Exception as e:
        logger.error(
            f"Unexpected error creating 'vw_autoremove_candidates_latest_scan' in {db_path}: {e}"
        )
        ...
        for file_name in files_in_dir:
            ...
            try:
                ...
                if os.path.isfile(full_path_str) and should_process_file(
                    Path(full_path_str), stat_result, config
                ):
    ...
    with tqdm(
        ...
        for full_path_str, stat_result in paths_to_process_with_stats:
            ...
            if cache_key in hash_cache:
                ...
                hash_algorithm = cached_data.get(
                    "hash_algorithm"
                ) or infer_algorithm_from_hash(cached_data["hash"])
                ...
                if abs(cached_mtime - mtime) <= 2:
                    ...
                    if hash_algorithm == "md5":
                        logger.info(
                            f"Upgrading hash for {relative_path} from MD5 to XXHash64"
                        )
                        ...
                        cache_hit = True
                        logger.trace(
                            f"Cache hit for {relative_path}: hash {file_hash} using {hash_algorithm}"
                        )
                else:
                    logger.debug(
            ...
            if not cache_hit:
                ...
                try:
                    file_hash, hash_algorithm = get_file_hash_with_algorithm(
                        Path(full_path_str), "xxh64", no_progress=no_progress
                    )
                    if file_hash:
                        ...
                        logger.debug(
                            f"Updated in-memory cache for {relative_path} with new {hash_algorithm} hash {file_hash}"
                        )
                        if use_sqlite:
            ...
            if "hash_algorithm" in cached_entry:
                ...
                try:
                    hash_algorithm = infer_algorithm_from_hash(
                        cached_entry.get("hash", "")
                    )
                except ValueError:
...
def find_orphaned_files(
    skip_media_check: bool = False, use_sqlite: bool = False, no_progress: bool = False
) -> int:
    ...
    if use_sqlite:
        scan_id_from_db, _ = save_scan_results_to_db(
...
def save_scan_results_to_db(
    ...
    try:
        ...
        for file_info_ot_pre_check in orphaned_torrent_files:
            path_ot_pre_check = (
                file_info_ot_pre_check["path"]
                if isinstance(file_info_ot_pre_check, dict)
                else file_info_ot_pre_check
            )
        ...
        for file_info in orphaned_torrent_files:
            ...
            if existing_record:
                ...
                logger.debug(
                    ...
                    f"Consecutive_scans: 1. Status 'active'."
                )
        ...
        for file_info in only_in_torrents:
            ...
            if os.path.exists(full_path):
                try:
                    ...
                    logger.error(
                        f"Error getting file hash from cache for 'torrents' source: {e}"
                    )
        ...
        for file_info in only_in_media:
            ...
            if os.path.exists(full_path):
                try:
                    ...
                    logger.error(
                        f"Error getting file hash from cache for 'media' source: {e}"
                    )
            ...
            with sqlite3.connect(str(config.sqlite_cache_path)) as conn:
                ...
                with tqdm(
                    ...
                    for relative_path, data in hash_cache.items():
                        ...
                        file_size = (
                            full_path.stat().st_size if full_path.exists() else 0
                        )
...
def get_files_to_mark_for_deletion(db_path: Path) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        logger.debug(
            f"Found {len(files_to_mark)} files eligible to be marked for deletion."
        )
    except sqlite3.Error as e:
...
def process_deletions(
    force_delete: bool, db_path: Path, torrent_base_folder: Path
) -> None:
    """
    ...
    if force_delete:
        ...
        try:
            ...
            logger.error(
                f"SQLite error fetching active torrent orphans for force deletion: {e}"
            )
            return  # Can't proceed if we can't fetch

        if not files_to_remove_directly:
            logger.info(
                "No 'active' orphaned files from 'local_torrent_folder' source found to force delete."
            )
            return

        logger.info(
            f"Found {len(files_to_remove_directly)} active torrent orphans for immediate deletion."
        )
        deleted_count = 0

        if not torrent_base_folder.exists() or not torrent_base_folder.is_dir():
            logger.error(
                f"Provided torrent_base_folder '{torrent_base_folder}' does not exist or is not a directory. "
                f"Cannot proceed with deletions."
            )
            return
            ...
            for file_info in files_to_remove_directly:
                ...
                try:
                    if absolute_file_path.exists():
                        ...
                        logger.success(
                            f"Successfully force-deleted file: {absolute_file_path}"
                        )
                        cursor.execute(
                            """
                            UPDATE orphaned_files
                        ...
                        logger.warning(
                            f"File not found during force delete: {absolute_file_path}. "
                            f"Updating status to 'deleted' as it's gone."
                        )
                    ...
                    FileNotFoundError
                ):  # Should be caught by .exists(), but as a fallback
                    logger.warning(
                    ...
                    logger.error(
                        f"Permission denied. Cannot force delete file: {absolute_file_path}"
                    )
                except OSError as e:
                    logger.error(
                        f"OS error during force delete of file {absolute_file_path}: {e}"
                    )
                except sqlite3.Error as e:
                    logger.error(
        ...
        logger.info(
            "Dry run for deletions. Identifying files eligible for deletion and marking them..."
        )
        files_to_mark = get_files_to_mark_for_deletion(db_path)
        if not files_to_mark:
            logger.info(
                "No new files eligible to be marked for deletion based on current criteria."
            )
            return
        ...
        with sqlite3.connect(str(db_path)) as conn:
            ...
            for file_info in files_to_mark:
                ...
                try:
                    ...
                    logger.error(
                        f"SQLite error marking file ID {file_id} ({file_path_str}) for deletion: {e}"
                    )
                    conn.rollback()
        logger.info(
            f"Marked {len(files_to_mark)} files for deletion. Run with --force to actually delete them."
        )
...
def execute_pending_actions(
    ...
    def apply_relabel_callback(
        file_path: str, action_params: str
    ) -> tuple[bool, str | None]:
        """
        ...
        try:
            ...
            if not torrent_id or not new_label:
                logger.error(
                    f"Missing required parameters (torrent_id or label) for relabeling: {action_params}"
                )
            ...
            try:
                ...
                if not status:  # Empty dict if torrent doesn't exist
                    logger.warning(
                        f"Cannot apply label '{new_label}' to torrent ID {torrent_id} as it no longer exists in Deluge."
                    )
            ...
            logger.info(
                f"Applied label '{new_label}' to torrent ID {torrent_id} (file: {file_path})"
            )
            return (True, None)
    ...
    execute_all_pending_actions(
        db_path=db_path,
        apply_relabel_callback=apply_relabel_callback,
        delete_file_callback=delete_file_callback,
        dry_run=dry_run,
    )
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Deluge Orphaned Files - Find and manage orphaned torrent files",
        epilog="For more information, visit https://github.com/FiveBoroughs/deluge-orphaned-files",
    )
    parser.add_argument(
    ...
    results_group.add_argument(
        "--list-scans",
        action="store_true",
        help="List all previous scans with IDs and timestamps",
    )
    results_group.add_argument(
        "--scan-id", type=int, help="Show detailed results for a specific scan ID"
    )
    results_group.add_argument(
        "--show-last", action="store_true", help="Show results for the most recent scan"
    )
    results_group.add_argument(
        "--limit",
        type=int,
    ...
    file_group.add_argument(
        ...
        action="store_true",
        help=f"Apply '{config.deluge_autoremove_label}' label in Deluge to torrents found only on torrent disk "
        "(not in media) and not already labeled as such. Performs a dry run if not specified.",
    )
    file_group.add_argument(
    ...
    log_group.add_argument(
        "--trace", action="store_true", help="Enable trace logging (most verbose)"
    )
    log_group.add_argument(
        "--list-env",
        dest="list_env",
    ...
    if args.test_email:
        if (
            ...
            and config.smtp_from_addr
            and config.smtp_to_addrs
        ):
            test_body = (
            ...
            logger.error(
                "Cannot send test e-mail: incomplete SMTP configuration (check SMTP_* env vars)."
            )
        return
    ...
    if args.test_telegram:
        if config.telegram_bot_token and config.telegram_chat_id:
            ...
            logger.error(
                "Cannot send test Telegram message: TELEGRAM_BOT_TOKEN or TELEGRAM_CHAT_ID not configured."
            )
        return
    ...
    if args.show_last or args.scan_id is not None:
        if not args.sqlite:
            logger.error(
                "Cannot show scan results without --sqlite flag, as data is in the database."
            )
            return
        if not os.path.exists(str(config.sqlite_cache_path)):
            logger.error(
        ...
        results = get_formatted_scan_results(
            args.scan_id if args.scan_id is not None else None
        )
        print(results)
        return
    ...
    if args.list_scans:
        if not args.sqlite:
            logger.error(
                "Cannot list scans without --sqlite flag, as data is in the database."
            )
            return
        if not os.path.exists(str(config.sqlite_cache_path)):
            logger.error(
    ...
    if args.clean_cache:
        if not args.sqlite:
            ...
            if not os.path.exists(str(config.sqlite_cache_path)):
                logger.error(
                    f"SQLite database not found at {str(config.sqlite_cache_path)} for cache cleaning."
                )
                return
    ...
    if args.execute_pending_actions:
        ...
        logger.info(
            "Skipping processing of pending actions. Use --execute-pending-actions to check and execute pending actions."
        )

    if not args.sqlite:
        logger.warning(
            "Skipping '{}' labeling because --sqlite is not enabled; database-backed scan results are required.",
            config.deluge_autoremove_label,
        )
    else:
        deluge_client = None
        try:
            ...
            if deluge_client.connected:
                ...
                process_autoremove_labeling(
                    ...
                    dry_run=not args.apply_autoremove_labels,
                ...
                if args.execute_pending_actions:
                    ...
                    if not args.force:
                        logger.info(
                            "Pending actions were only processed in dry-run mode. Use --force to actually apply changes."
                        )

            else:
                logger.error(
                    f"Failed to connect to Deluge for labeling. Skipping '{config.deluge_autoremove_label}' labeling."
                )
            ...
            logger.error(
                f"An error occurred during '{config.deluge_autoremove_label}' labeling: {e}. Skipping."
            )
        finally:
    ...
    if args.sqlite:
        if scan_id > 0:
            ...
            logger.info(
                f"To view these results later, use: --scan-id {scan_id} --sqlite"
            )
        ...
        logger.info(f"Scan results saved to {config.output_file}")
        logger.info(
            "Deletion processing (--force) is only available with --sqlite mode."
        )
    ...
    if (
        ...
        logger.debug(
            "SMTP configuration incomplete or e-mail disabled; skipping notification."
        )
    ...
    if scan_id > 0 and config.telegram_bot_token and config.telegram_chat_id:
        ...
        logger.debug(
            "Telegram configuration incomplete or notification disabled; skipping notification."
        )

    # Only show the formatted table from the database when using SQLite mode
    if scan_id > 0:
        try:
            summary_table = _format_scan_results(
                config.sqlite_cache_path, scan_id=scan_id
            )
            logger.info("\n{}", summary_table)
        except Exception as exc:  # noqa: BLE001
