...
if LOCAL_TORRENT_BASE_LOCAL_FOLDER:
    logger.info(f"Found LOCAL_TORRENT_BASE_LOCAL_FOLDER in .env: {LOCAL_TORRENT_BASE_LOCAL_FOLDER}")
else:
...
if LOCAL_TORRENT_BASE_LOCAL_FOLDER:
    ...
    logger.warning(f"LOCAL_TORRENT_BASE_LOCAL_FOLDER not found in .env, using default database path: {DEFAULT_SQLITE_PATH}")
...
def get_sql_results(db_path):
    ...
    try:
        ...
        if not cursor.fetchone():
            logger.error(f"'vw_latest_scan_report' view not found. Run the main script to initialize/update the DB.")
            sys.exit(1)
        
        # Get all data from the latest scan using the view
        cursor.execute("""
        SELECT scan_id, scan_host, scan_base_path, scan_start, scan_end, 
               file_path, file_label, file_size, file_size_human, scan_context_file_source
        ...
        # Extract scan metadata from the first row (it will be the same for all rows from the view)
        ...
        logger.info(f"Successfully retrieved {len(all_rows_from_view)} files from the latest scan (ID: {latest_scan_id}) using vw_latest_scan_report")
...
def compare_results(json_data, sql_data, output_file=None):
    ...
    output = []
    output.append("\n=== COMPARISON RESULTS ===\n")
    output.append(f"JSON scan time: {json_data.get('scan_start', 'N/A')} to {json_data.get('scan_end', 'N/A')}")
    output.append(f"SQL scan time: {sql_data.get('scan_start', 'N/A')} to {sql_data.get('scan_end', 'N/A')}")
    ...
    categories = [
        ("in_local_torrent_folder_but_not_deluge", "Files in local torrent folder but not in Deluge"),
    ...
    for category_key, description in categories:
        ...
        if attribute_mismatches_details:
            output.append(f"  Attribute mismatches for common files ({category_attribute_mismatches}):")
    ...
    output.append(f"Total files in SQL but not in JSON: {overall_summary['total_missing_in_json']}")
    output.append(f"Total common files checked for attributes: {overall_summary['total_common_files_checked']}")
    output.append(f"Total attribute mismatches in common files: {overall_summary['total_attribute_mismatches']}")
...
def main():
    ...
    parser.add_argument("--db_path", default=DEFAULT_SQLITE_PATH, help="Path to the SQLite database file")
    parser.add_argument("--output", default="comparison_results.txt", help="File to save the comparison results")
    ...
    logger.info(f"Starting comparison with the following files:")
    logger.info(f"  JSON file (absolute): {abs_json_path}")
